  ...
  const [markers, setMarkers] = useState<MarkerType[]>([]); // State to store markers
    ...
    const fetchMarkers = async (serviceChosen: string) => {
      try {
        ...
        const response = await fetch(`https://db-production-c620.up.railway.app/marker/getService/${serviceChosen}`);
        ...
        if (Array.isArray(data)) {
          ...
          if (location) {
            ...
            if (filteredMarkers.length > 0) {
              ...
              const fallbackResponse = await fetch(`https://db-production-c620.up.railway.app/marker/getStation/${serviceChosen}`);
    ...
    const updateStatusRequest = async (status: string, userId: number) => {
      try {
          const response = await axios.put(`https://db-production-c620.up.railway.app/servicerequest/updateRequest/${status}`, {
              UserID: userId
          }, {
              headers: {
      ...
      try {
        ...
        if (requestType === 'Canceled Service') {
          if (USERID !== null) {
            ...
            const markerUpdateResponse = await axios.put(`https://db-production-c620.up.railway.app/marker/updateMarkerTitle/${USERID}`, {
        ...
        const serviceRequestResponse = await axios.post('https://db-production-c620.up.railway.app/servicerequest/submit', {
          UserID: USERID,
          Username: username,
